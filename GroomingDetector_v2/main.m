...
%     OutputNframe = floor(Duration/TimeInterval/AnalyzingStep)+1;
...
%     end
    ...
    PreviousPeriphery=zeros(Nrow,Ncolumn,int16(Yinterval),int16(Xinterval));
    PreviousCore=zeros(Nrow,Ncolumn,int16(Yinterval),int16(Xinterval));
    ...
    while hasFrame(mov)
        ...
        CurrentFrame=readFrame(mov);
        CurrentFrame=rgb2gray(CurrentFrame);
        
        index1=index1+1;
        CurrentBG = BackgroundSet(:,:,floor(mov.CurrentTime/BackgroundUpdate*FrameRate)+1);
        
        if TransVideo==1
            CurrentFrame=CurrentFrame';
        end
        Diff = uint8(zeros(size(CurrentFrame)));
        Diff(CurrentBG - CurrentFrame >= C0) = 1;
        ...
        for nOverSize=1:length(OverSize)
            Diff(CC.PixelIdxList{OverSize(nOverSize)})=0; 
        end
        for ncol = 1:Ncolumn
            for nrow = 1:Nrow
...
%               try
%                   Diff(floor(Yposition(nrow,ncol,index1))-3:floor(Yposition(nrow,ncol,index1))+3,floor(Xposition(nrow,ncol,index1))-3:floor(Xposition(nrow,ncol,index1))+3)=0;
%                   Diff(floor(Yposition(nrow,ncol,index1))-3:floor(Yposition(nrow,ncol,index1))+3,floor(Xposition(nrow,ncol,index1))-3:floor(Xposition(nrow,ncol,index1))+3)=0;
%               end
            end
        end
